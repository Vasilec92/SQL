...
-- Вариант на JOIN
SELECT users.first_name, users.last_name, cities.name AS city, media.filename AS photo
  FROM users
    JOIN profiles
      ON users.id = profiles.user_id
    JOIN cities
      ON cities.id = profiles.city_id    
    JOIN media
...
SELECT DISTINCT media.user_id, media.filename, media.created_at
  FROM media
    ...
    JOIN users 
      ON (users.id = friendships.friend_id 
        OR users.id = friendships.user_id)
      AND (media.user_id = friendships.user_id 
        OR media.user_id = friendships.friend_id)   
  WHERE users.id = 7 AND media.user_id != 7;   

-- Проверка
SELECT user_id, friend_id FROM friendships WHERE user_id = 7 OR friend_id = 7;
...
SELECT media.id, media.user_id, media.filename, media.created_at
  FROM media
    ...
    JOIN users 
      ON users.id = friendships.friend_id
        OR users.id = friendships.user_id   
  WHERE users.id = 7 ORDER BY media.id;
...
-- Выполним объединение без группировки и посмотрим на результат
SELECT users.id, first_name, last_name, friendships.created_at
  FROM users
...
-- Затем подсчитаем
SELECT users.id, first_name, last_name, COUNT(friendships.created_at) AS total_friends
  FROM users
    LEFT JOIN friendships
      ON users.id = friendships.user_id
        OR users.id = friendships.friend_id
...
SELECT likes.target_id,
  ...
  WHERE users.id = 4 AND target_types.name = 'media'
  GROUP BY media.id;
